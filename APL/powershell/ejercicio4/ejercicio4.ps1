...
$pidFile = "$env:TEMP\audit_$(($repo -replace '[\\/:]','_')).pid"
$alertDb = "$env:TEMP\audit_alerts_$(($repo -replace '[\\/:]','_')).txt"
...
if (Test-Path $log) {
    ...
    if ($logDir) {
        ...
        $log = (Resolve-Path (Get-Location).Path -ErrorAction Stop).Path + "\" + (Split-Path $log -Leaf)
    }
}
